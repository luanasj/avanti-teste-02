...
import { githubSearch, githubUser } from "../../services/githubService"

const SearchBar = ({placeholder,setResult,setIsLoading})=>{
   const [name,setName] = useState("")
   
    const servicesRequestsHandler = async (user_name)=>{
        ...
        const searchResult = await githubSearch(user_name)
    ...
    return(
        <form className="bg-white w-[503px] h-[62px] rounded-[10px] border-1 border-[#DDDDDD] flex items-center">
            ...
            <button type="submit" onClick={submitOnClickHandler} className="rounded-[10px] border-1 border-r-0 border-[#DDDDDD] size-[62px] flex items-center justify-center shrink-0 bg-[#005CFF] cursor-pointer">
